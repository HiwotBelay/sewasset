...
export const ResultsDashboard = forwardRef<HTMLDivElement, ResultsProps>(({ results }, ref) => {
    ...
    <div ref={ref} className="space-y-6">
      ...
      <Card className="p-8 bg-gradient-to-br from-[#2E4059] to-[#3B5998] text-white">
        <div className="flex justify-between items-start mb-6">
          <div>
            <p className="text-white/80 text-sm mb-2">Your Return on Investment</p>
            <div className="flex items-baseline gap-2">
              <span className={`text-5xl font-bold ${isPositive ? "text-[#FDC700]" : "text-red-400"}`}>
                {isPositive ? "+" : ""}
                {results.roi}%
              </span>
              <span className={`text-xl ${isPositive ? "text-[#10B981]" : "text-red-400"}`}>
                {isPositive ? "📈" : "📉"}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-6 bg-white border border-[#E5E7EB]">
          ...
          <p className="text-2xl font-bold text-[#2E4059]">${Number.parseFloat(results.gain).toLocaleString()}</p>
        </Card>
        <Card className="p-6 bg-white border border-[#E5E7EB]">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold text-[#2E4059]">${Number.parseFloat(results.totalCost).toLocaleString()}</p>
        </Card>
        <Card className="p-6 bg-white border border-[#E5E7EB]">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold text-[#2E4059]">{results.annualROI}%</p>
        </Card>
      </div>
      ...
      <Card className="p-6 bg-white">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={results.projections}>
            ...
            <YAxis stroke="#9CA3AF" />
            <Tooltip contentStyle={{ backgroundColor: "#F8F9FA", border: "1px solid #E5E7EB", borderRadius: "8px" }} />
            <Area
              type="monotone"
              dataKey="value"
              stroke="#FDC700"
